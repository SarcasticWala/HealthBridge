...
const Navbar = () => {
    ...
    const [token, setToken] = useState(true);//  token 1 logged in | token 0 logged out
    return (
        ...
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
                    ...
                    <div className='flex items-center gap-2 cursor-pointer group relative'>
                        ...
                        <div className='absolute top-0 right-0 pt-14 text-base font-medium  text-gray-600 z-20 hidden group-hover:block'>
                            <div className='min-w-48 bg-stone-100 flex flex-col gap-4 p-4'>
                                ...
                                <p onClick={()=>setToken(false)} className="cursor-pointer hover:text-primary">Logout</p>
                            </div>
                        </div>
                    </div>
                ) : (
            ...
            <div className={`${showMenu ? 'fixed w-full h-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all duration-300`}>
                ...
                <NavLink  onClick={()=>setshowMenu(false)} to='/doctors'><p className='px-4 py-2 rounded inline-block hover:bg-gray-200 transition duration-200'>ALL DOCTORS</p></NavLink>
                <NavLink onClick={()=>setshowMenu(false)} to='/about'><p className='px-4 py-2 rounded inline-block hover:bg-gray-200 transition duration-200'>ABOUT</p></NavLink>
                <NavLink  onClick={()=>setshowMenu(false)} to='/contact'><p className='px-4 py-2 rounded inline-block hover:bg-gray-200 transition duration-200'>CONTACT</p></NavLink>
                </ul>
            </div>
            </div>
        </div>
